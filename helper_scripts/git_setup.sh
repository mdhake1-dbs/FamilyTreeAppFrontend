...
# Load credentials
if [ ! -f $HOME/family-tree-vault/helper_scripts/github_credentials.env ]; then
  echo "github_credentials.env not found! Create it first."
  exit 1
fi
source $HOME/family-tree-vault/helper_scripts/github_credentials.env
...
REMOTE_URL="https://${GITHUB_USERNAME}/${REPO_NAME}.git"
